...
restart:
	...
	DLOG(SHOW_UPDATES, "update queued %d %d %d %d %d\n",
		...
		msmfb->yoffset);
	/* Jay, update panel data, the mddi client needs that */
	memcpy(&panel->update_info, &msmfb->update_info, sizeof(panel->update_info));
...
static void msmfb_earlier_suspend(struct early_suspend *h)
{
	...
	/* inform power that we will goto suspend */
	rc = batt_notifier_call_chain(BATT_EVENT_SUSPEND, (void *)msmfb);
	if (rc == NOTIFY_STOP) {
		DLOG(SUSPEND_RESUME, "%s: prevent suspend, htc_power_policy=1\n", __func__);
		return;
	}
...
static int msmfb_probe(struct platform_device *pdev)
{
	...
	if (!load_565rle_image(INIT_IMAGE_FILE)) {
		...
		msmfb->update_info.eright = msmfb->fb->var.xres;
		msmfb->update_info.ebottom = msmfb->fb->var.yres;
		msmfb_pan_update(msmfb->fb, 0, 0, msmfb->fb->var.xres,
				msmfb->fb->var.yres, 0, 1);
	}
#endif
	return 0;

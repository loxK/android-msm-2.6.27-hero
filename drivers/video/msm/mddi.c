...
static void mddi_wait_interrupt(struct mddi_info *mddi, uint32_t intmask)
{
	if (mddi_wait_interrupt_timeout(mddi, intmask, HZ/10) == 0)
		printk(KERN_INFO KERN_ERR "%s PID: %d, timeout waiting for "
		       "%x, INT = %x, STAT = %x gotint = %x\n", __func__, 
			...
			uint32_t reg, unsigned int nr_bytes)
{
	...
	ll->flags = 1;
	ll->header_count = 14; /* register access packet header occupies 14 bytes */
...
void 
mddi_remote_write(struct msm_mddi_client_data *cdata, uint32_t val, uint32_t reg)
{
...
/*
 * This fixup should be for each client, but since we did not know client yet,
 * it locates in board.
 *
 * Jay, 12/1/08' 
 */
static void __init 
mddi_fixup(struct msm_mddi_platform_data * platform, struct mddi_info * mddi)
{
	if (platform->fixup)
		platform->fixup(&mddi->caps.Mfr_Name, &mddi->caps.Product_Code);
	printk(KERN_DEBUG "%s: mfr_name = 0x%x, product_code = 0x%x\n",
		__func__, mddi->caps.Mfr_Name, mddi->caps.Product_Code);
}
...
static int __init mddi_probe(struct platform_device *pdev)
{
	...
	mddi->base = ioremap(resource->start, resource->end - resource->start + 1);
	if (!mddi->base) {
		ret = -EINVAL;
		goto error_ioremap;
	}
	...
	/* Jay, for unknown bridge */
	printk("mfr_name = %d, product_code = %d\n", mddi->caps.Mfr_Name,
						mddi->caps.Product_Code);
	if (mddi->caps.Mfr_Name == 0 && mddi->caps.Product_Code == 0) {
		mddi_fixup(pdata, mddi);
	}
	...
	/* Jay, for samsung that needs write bunch of vals for one cmd */

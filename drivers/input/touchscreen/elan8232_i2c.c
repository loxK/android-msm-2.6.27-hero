...
static int ekt8232_ts_init(struct i2c_client *client)
{
	...
	if (ekt_data.fw_ver == 0x103) {
		...
		if ((i2c_master_send(client, cmd, 4)) != 4) {
			dev_err(&client->dev, "%s: set adc failed\n", __func__);
		}
		cmd[0] = 0x54;
		cmd[0] = 0x43;
		cmd[0] = 0x00;
...
static void ekt8232_report_data(struct i2c_client *client, uint8_t *buf)
{
	...
	case idx_coordinate_packet: {
		...
		if (ime_threshold > 0) {
			...
			if ((x1 >= ime_work_area_x1 &&
			     x1 <= ime_work_area_x2) &&
			    (y1 >= ime_work_area_y1 &&
			     y1 <= ime_work_area_y2)) {
				dx = (x1 > report_x[0])	?
				     (x1 - report_x[0]): (report_x[0] - x1);
				dy = (y1 > report_y[0])	?
				     (y1 - report_y[0])	: (report_y[0] - y1);

				if ((dx > pixel_x * ime_threshold) ||
				    (dy > pixel_y * ime_threshold)) {
					if ((finger_stat == 1) &&
					    (report_x[0] != 0) &&
					    (report_y[0] != 0))
						touch_hit = 1;
		...
		if (point_moved) {
			if (finger_stat != 0) {
				...
				if (ekt_data.fw_ver == 0x101)
					...
					input_report_abs(ekt_data.input,
							 ABS_Y, y1);
			...
			input_report_key(ekt_data.input, BTN_TOUCH,
					 finger_stat);
			input_report_key(ekt_data.input, BTN_2,
					 finger_stat == 2);
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ekt_data.fw_ver >= 0x104) {
		...
		input_set_abs_params(ekt_data.input, ABS_HAT0Y,
				     ...
				     ELAN_TS_FUZZ, ELAN_TS_FLAT);
#ifdef ENABLE_IME_IMPROVEMENT
		touch_screen_margin_left = pdata->abs_x_min;
		touch_screen_margin_right = x_max;
		touch_screen_margin_top = pdata->abs_y_min;
		touch_screen_margin_bottom = y_max;
#endif
	} else {
		input_set_abs_params(ekt_data.input, ABS_X,
...
#ifdef ENABLE_IME_IMPROVEMENT
		touch_screen_margin_left = pdata->abs_x_min;
		touch_screen_margin_right = pdata->abs_x_max;
		touch_screen_margin_top = pdata->abs_y_min;
		touch_screen_margin_bottom = pdata->abs_x_max;
#endif
	}
	...
	err = device_create_file(&ekt_data.input->dev, &dev_attr_ime_threshold);
	if (err) {
		printk(KERN_ERR "ENABLE_IME_IMPROVEMENT: "
				"Error to create ime_threshold\n");
		goto fail;
	}
	err = device_create_file(&ekt_data.input->dev, &dev_attr_ime_work_area);
	if (err) {
		printk(KERN_ERR "ENABLE_IME_IMPROVEMENT: "
				"Error to create ime_work_area\n");
		device_remove_file(&ekt_data.input->dev,
...
#ifdef ENABLE_IME_IMPROVEMENT
	device_remove_file(&ekt_data.input->dev, &dev_attr_ime_threshold);
	device_remove_file(&ekt_data.input->dev, &dev_attr_ime_work_area);
#endif

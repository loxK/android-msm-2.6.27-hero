...
#define CY8C_REG_INTR_STATUS    0x39
...
static const char CYPRESSNAME[]	= "Cypress-tst110";
uint32_t cy8c_panel_version;
		...
		struct device_attribute *attr, char *buf)
{
	...
	sprintf(buf, "%s_%#x\n", CYPRESSNAME, cy8c_panel_version);
	ret = strlen(buf) + 1;
...
/*
static s32 cy8c_write_word_data(struct i2c_client *client,
                                u8 command, u16 value)
{
	u16 data = (value << 8) | (value >> 8);
...
static int cy8c_init_panel(struct cy8c_ts_data *ts)
{
	int ret;
	 ...
	 /* clean intr busy */
	ret = i2c_smbus_write_byte_data(ts->client, CY8C_REG_INTR_STATUS, 0x00);
	if (ret < 0) {
	 ...
	 /* start new scan */
	ret = i2c_smbus_write_byte_data(ts->client, CY8C_REG_START_NEW_SCAN, 0x01);
	if (ret < 0) {
		dev_err(&ts->client->dev,
...
{
	int ret, finger2_pressed = 0;
	uint8_t start_reg;
	uint8_t buf[11];
	uint8_t force, area, is_touch;
...
{
	struct cy8c_ts_data *ts = container_of(timer, struct cy8c_ts_data, timer);
	...
	, const struct i2c_device_id *id)
{
	...
	ts->cy8c_wq = create_singlethread_workqueue("cypress_wq");
	if (!ts->cy8c_wq) {
		ret = -ENOMEM;
	...
	cy8c_panel_version = panel_version;
	if (pdata) {
	...
	if (client->irq) {
		ret = request_irq(client->irq, cy8c_ts_irq_handler, IRQF_TRIGGER_FALLING,
...
static int cy8c_ts_resume(struct i2c_client *client)
{
	...
	if (ts->power) {
		ret = ts->power(1);
		if (ret < 0)
			dev_err(&client->dev, "cy8c_ts_resume power on failed\n");
    	msleep(10);

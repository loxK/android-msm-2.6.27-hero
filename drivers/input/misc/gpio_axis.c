...
static enum hrtimer_restart emc_progress_hrtimer_handler_func(struct hrtimer *timer)
{
	...
	if(dmc_gpio_status == ai->emc_gpio_state) {
		...
		//printk(KERN_DEBUG "hrtimer_handler: old: %d, new: %d (%d, %d)@.@\n",
		//	ai->emc_gpio_state, dmc_gpio_status, ai->gpio[0], ai->gpio[1]);
	}
	...
	for (i = ai->count - 1; i >= 0; i--) {
		if(atomic_read(&ai->emc_disable_irqnum) & (1 << i)) {
			...
			atomic_set(&ai->emc_disable_irqnum,
				atomic_read(&ai->emc_disable_irqnum) & ~(1 << i));
		}
	}
...
static irqreturn_t gpio_axis_irq_handler(int irq, void *dev_id)
{
	...
	if(ai->enable_emc_protect_delay) {
		...
		for (i = ai->count - 1; i >= 0; i--) {
			ai->emc_gpio_state =
				(ai->emc_gpio_state << 1) | gpio_get_value(ai->gpio[i]);			
		...
		if(!atomic_read(&as->atomic_emc_hrtimer_is_run)) {
			...
			hrtimer_start(&as->emc_hrtimer_delay,
				ktime_set(0, ai->enable_emc_protect_delay), HRTIMER_MODE_REL);
		}
	} else
		gpio_event_update_axis(as, 1);
	return IRQ_HANDLED;
	...
	if (func == GPIO_EVENT_FUNC_INIT) {
		*data = as = kmalloc(sizeof(*as), GFP_KERNEL);
		...
		if(ai->enable_emc_protect_delay) {
			...
			hrtimer_init(&as->emc_hrtimer_delay, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
			as->emc_hrtimer_delay.function = emc_progress_hrtimer_handler_func;

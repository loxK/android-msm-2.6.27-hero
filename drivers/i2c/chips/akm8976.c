...
static int AKI2C_RxData(char *rxData, int length)
{
	...
	printk(KERN_ERR"AKI2C_RxData retry over %d times\n", I2C_RETRY_TIMES);
...
static int AKI2C_TxData(char *txData, int length)
{
	...
	printk(KERN_ERR"AKI2C_TxData retry over %d times\n", I2C_RETRY_TIMES);
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = akm8976_init_client(client);
	if (err < 0) {
		printk(KERN_ERR"akm8976_probe: Failed to init client\n");

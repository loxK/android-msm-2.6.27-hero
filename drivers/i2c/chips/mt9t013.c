...
#include <mach/mt9t013.h> /* define ioctls */
...
static struct msm_camera_device_platform_data  *cam3M;
...
static int mt9t013_i2c_exposure_gain(uint32_t mode, uint16_t line, uint16_t gain);
...
static void mt9t013_sensor_init(void)
{
	...
	ret = gpio_request(cam3M->sensor_reset, "mt9t013");
	if (!ret) {
		gpio_direction_output(cam3M->sensor_reset, 1);
		printk(KERN_INFO "mt9t013: camera sensor_reset set as 1\n");
	} else
		printk(KERN_ERR "mt9t013 error: request gpio %d failed: "
				"%d\n", cam3M->sensor_reset, ret);
	mdelay(2);

	/* pull down power down */
	ret = gpio_request(cam3M->sensor_pwd, "mt9t013");
	if (!ret || ret == -EBUSY)
		gpio_direction_output(cam3M->sensor_pwd, 0);
	else printk(KERN_ERR "mt913t013 error: request gpio %d failed: "
			"%d\n", cam3M->sensor_pwd, ret);
	gpio_free(cam3M->sensor_pwd);
	...
	/* enable gpio */
	cam3M->config_gpio_on();

	/* delay 2 ms */
	mdelay(2);

	/* reset sensor sequency */
	gpio_direction_output(cam3M->sensor_reset, 0);
	mdelay(2);
	gpio_direction_output(cam3M->sensor_reset, 1);
	gpio_free(cam3M->sensor_reset);
	mdelay(2);
...
static int mt9t013_lens_power(int on)
{
	...
	rc = gpio_request(cam3M->vcm_pwd, "mt9t013");
	if (!rc)
		gpio_direction_output(cam3M->vcm_pwd, !on);
	else printk(KERN_ERR "mt9t013 error: request gpio %d failed:"
		" %d\n", cam3M->vcm_pwd, rc);
	gpio_free(cam3M->vcm_pwd);
	return rc;
}
...
static int mt9t013_i2c_exposure_gain(uint32_t mode, uint16_t line, uint16_t gain)
{
  ...
  if (mode == 1) {
    I2C_WRITE(MT9T013_REG_RESET_REGISTER, 0x10cc|0x0002); //RESET REGISTER RESTART
  }
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	cam3M = client->dev.platform_data;
...
{
	return i2c_add_driver(&mt9t013_driver);
}

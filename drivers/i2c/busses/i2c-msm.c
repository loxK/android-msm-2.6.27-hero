...
static int
msm_i2c_poll_notbusy(struct msm_i2c_dev *dev, uint16_t addr)
{
	uint32_t retries = 0;
...
msm_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	...
	ret = msm_i2c_poll_notbusy(dev, msgs->addr);
	if (ret) {
	...
	udelay(20);
	/* Read may not have stopped in time */
	if (msm_i2c_poll_notbusy(dev, msgs->addr))

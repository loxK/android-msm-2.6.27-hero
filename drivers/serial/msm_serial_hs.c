...
struct msm_hs_port {
	...
	struct perf_lock serial_perf_lock;
	#endif
};
			 ...
			 const char *buf, size_t count)
{
	...
	if(serial_lock_cpu_state) {	//lock cpu
		if(msm_uport->cpu_lock_supported) {
			if(!msm_uport->is_cpu_lock) {
				msm_uport->is_cpu_lock = 1;
				perf_lock(&msm_uport->serial_perf_lock);
				printk("====== Serial LOCKs CPU ======\n");
			}
			else
				printk("====== WARNING!! Serial LOCKs CPU but it was locked before!! ======\n");
		}
	} else {	//unlock cpu
		if(msm_uport->cpu_lock_supported) {
			if(msm_uport->is_cpu_lock) {
				perf_unlock(&msm_uport->serial_perf_lock);
				msm_uport->is_cpu_lock = 0;
				printk("====== Serial UNLOCKs CPU ======\n");
			}
			else
				printk("====== WARNING!! Serial UNLOCKs CPU but it was unlocked before!! ======\n");
		}
	}
...
static int msm_uartdm_ioctl(struct uart_port *uport, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		#ifdef SERIAL_CPU_LOCK_SUPPORTED
		case 0x8011:	//lock cpu
			if(msm_uport->cpu_lock_supported) {
				if(!msm_uport->is_cpu_lock) {
					msm_uport->is_cpu_lock = 1;
					perf_lock(&msm_uport->serial_perf_lock);
					printk("------ ioctl Serial LOCKs CPU ------\n");
				}
				else
					printk("------ WARNING!! Serial LOCKs CPU but it was locked before!! ------\n");
			}
		...
		case 0x8012:	//unlock cpu
			if(msm_uport->cpu_lock_supported) {
				if(msm_uport->is_cpu_lock) {
					perf_unlock(&msm_uport->serial_perf_lock);
					msm_uport->is_cpu_lock = 0;
					printk("------ ioctl Serial UNLOCKs CPU ------\n");
				}
				else
					printk("------ WARNING!! Serial UNLOCKs CPU but it was unlocked before!! ------\n");
			}
...
static int __init msm_hs_probe(struct platform_device *pdev)
{
	...
	if (pdata == NULL)
		...
		msm_uport->is_cpu_lock = 0;
		if(msm_uport->cpu_lock_supported) {
			perf_lock_init(&msm_uport->serial_perf_lock, PERF_LOCK_HIGHEST, "msm_serial_hs");	// cpu=528MHz
		}

			       ...
			       unsigned long flags)
{
	if ((flags & GPIOF_DRIVE_OUTPUT) && !vreg_h2w)
		vreg_h2w = vreg_get(0, TROUT_H2W_POWER_NAME);
...
static struct gpio_chip trout_h2w_gpio_chip = {
	.start = TROUT_GPIO_H2W_POWER,
	.end = TROUT_GPIO_H2W_POWER,
...
static int __init trout_init_gpio(void)
{
	...
	register_gpio_chip(&trout_h2w_gpio_chip);

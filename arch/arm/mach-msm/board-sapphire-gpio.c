...
static const int _g_CPLD_MISCn_Offset[]= {0x0A,		/*misc1 reg*/
						...
						0x06};		/*misc5 reg*/
/*CPLD INT Bank*/
static const int _g_INT_BANK_Offset[][3] = {{0x0E,0x08,0x0C}};		/*BANK0: int1 status, int2 level, int3 mask*/
...
static int sapphire_gpio_read(struct gpio_chip *chip, unsigned n)
{
	if(n < SAPPHIRE_GPIO_INT_B0_BASE)	/*MISCn*/
		...
		return !!( readb(CPLD_INT_LEVEL_REG_G(n)) & CPLD_GPIO_BIT_POS_MASK(n));
	return 0;
}

/*CPLD Write only register :MISC2, MISC3, MISC4, MISC5 => reg=0,2,4,6
Reading from write-only registers is undefined, so the writing value should be kept in shadow for later usage.*/
...
static int sapphire_gpio_configure(struct gpio_chip *chip, unsigned int gpio, unsigned long flags)
{
...
static int sapphire_gpio_get_irq_num(struct gpio_chip *chip, unsigned int gpio, unsigned int *irqp, unsigned long *irqnumflagsp)
{
	DBG("gpio=%d, l=0x%x\r\n", gpio, readb(SAPPHIRE_CPLD_INT_LEVEL));
...
{
	writeb(SAPPHIRE_INT_BIT_MASK(irq), CPLD_INT_STATUS_REG(irq));	/*write 1 to clear*/
}
...
/*Save current working sources before sleep, so we can restore it after resume.*/
static int sapphire_sysdev_suspend(struct sys_device *dev, pm_message_t state)
{
	sapphire_suspended = 1;
	/*save current masking*/
	sapphire_int_mask[0] = readb(SAPPHIRE_CPLD_BASE +  SAPPHIRE_GPIO_INT_B0_MASK_REG);
...
int sapphire_sysdev_resume(struct sys_device *dev)
{
	...
	writeb(sapphire_int_mask[0], SAPPHIRE_CPLD_BASE +  SAPPHIRE_GPIO_INT_B0_MASK_REG);

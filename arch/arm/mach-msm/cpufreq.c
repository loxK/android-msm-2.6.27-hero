...
static int __init msm_cpufreq_init(struct cpufreq_policy *policy)
{
	...
	if (cpufreq_frequency_table_cpuinfo(policy, table)) {
		policy->cpuinfo.min_freq = CONFIG_MSM_CPU_FREQ_SCALING_MIN;
		policy->cpuinfo.max_freq = CONFIG_MSM_CPU_FREQ_SCALING_MAX;
	}
	policy->min = CONFIG_MSM_CPU_FREQ_SCALING_MIN;
	policy->max = CONFIG_MSM_CPU_FREQ_SCALING_MAX;

...
if(!from_idle) {
	printk("[R] resume starts collpased=%d\n", collapsed);
}
...
static void msm_pm_power_off(void)
{
	...
	if (msm_wakeup_after)
        ...
        sys_sync();  /* make sure checkedpoint is written before shut down */
...
static void msm_pm_restart(char str)
{
	 ...
	 */
         ...
         //sys_sync();  /* make sure checkedpoint is written before restart */

#if 1 /* Use PROC_COMM to restart device for all cases. 
       * This makes A9 do some essential actions before restart.
       * During the restart:
       * if A9 crash and notifies A11, we should get device reset in handling A9 crash.
       * if A9 crash but not notify A11, we should get device reset via A9's watchdog reset.
       */

		...
		void *cmd_data, size_t cmd_size)
{
	...
	if (cmd_size != sizeof(jpeg_cmd_enc_cfg)) {
		pr_info("adsp: module %s: "
			"JPEG ENC CFG invalid cmd_size %d\n",
	...
	for (i = 0; i < num_frags; i += 2) {
		if (adsp_pmem_fixup(module,
			(void **)(&cmd->frag_cfg_part[i]), luma_size) ||
				adsp_pmem_fixup(module,
			(void **)(&cmd->frag_cfg_part[i+1]),
			chroma_size))
			return -1;
	}
					  ...
					  void *cmd_data, size_t cmd_size)
{
	...
	if (cmd_size != sizeof(jpeg_cmd_dec_cfg)) {
		printk(KERN_ERR "adsp: module %s:"
			" JPEG DEC CFG invalid cmd_size %d\n",
			module->name, cmd_size);
		return -1;
	}
				  ...
				  void *cmd_data, size_t cmd_size)
{
	...
	case JPEG_CMD_ENC_OP_CONSUMED:
	{
		...
		if (cmd_size != sizeof(jpeg_cmd_enc_op_consumed)) {
			printk(KERN_ERR "adsp: module %s:"
				" JPEG_CMD_ENC_OP_CONSUMED invalid size %d\n",
				module->name, cmd_size);
			return -1;
		}

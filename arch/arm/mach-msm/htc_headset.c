...
struct h2w_info {
	...
	struct hrtimer short_detection_timer;
	ktime_t short_debounce_time;
...
static irqreturn_t detect_irq_handler(int irq, void *dev_id)
{
	...
	if ((hi->htc_headset_flag == 0) ^ value2) {
		...
		hrtimer_start(&hi->short_detection_timer,
			      hi->short_debounce_time,
			      HRTIMER_MODE_REL);
	}
	return IRQ_HANDLED;
}
...
static enum hrtimer_restart short_timer_func(struct hrtimer *data)
{
...
static int h2w_probe(struct platform_device *pdev)
{
	...
	hi->short_debounce_time = ktime_set(0, 1000000); /*1ms*/
	...
	hrtimer_init(&hi->short_detection_timer,
		CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	hi->short_detection_timer.function = short_timer_func;

...
{
	mutex_lock(&pjack_info->mutex_lock);
	cnf_driver_event("H2W_extend_headset", &pjack_info->audio_jack_flag);
	mutex_unlock(&pjack_info->mutex_lock);
}
...
static int audiojack_probe(struct platform_device *pdev)
{
	...
	mutex_init(&pjack_info->mutex_lock);

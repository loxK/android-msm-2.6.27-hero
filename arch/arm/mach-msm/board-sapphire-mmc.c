...
extern int msm_add_sdcc_devices(unsigned int controller, struct mmc_platform_data *plat);
...
static int sapphire_wifi_status_register(void (*callback)(int card_present, void *dev_id), void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
{
	int rc;
	printk("sapphire_bt_fastclock_power on = %d\n",on);
	if (vreg_wifi_osc) {
		if (on) {
			rc = vreg_enable(vreg_wifi_osc);
			printk("BT vreg_enable vreg_mmc, rc=%d\n",rc);
			if (rc)
			{
			...
			{
				vreg_disable(vreg_wifi_osc);
				printk("BT disable vreg_wifi_osc.\n");
			}
			else
				printk("BT shouldn't disable vreg_wifi_osc. WiFi is using it!!\n");
		}
	}
...
int sapphire_wifi_reset(int on)
{
	printk("%s: %d\n", __func__, on);
...
int __init sapphire_init_mmc(unsigned int sys_rev)
{
	...
	msm_add_sdcc_devices(1, &sapphire_wifi_data);

	if (!opt_disable_sdcard)
		msm_add_sdcc_devices(2, &sapphire_sdslot_data);
	else

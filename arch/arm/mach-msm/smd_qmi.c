...
static int qmi_authentication = 3;
static int qmi_technology = 1;
...
static int qmi_network_up(struct qmi_ctxt *ctxt, char *apn)
{
	...
	msg.tlv = data + QMUX_HEADER;
	ctxt->wds_txn_id += 2;
	qmi_add_tlv(&msg, 0x30, 0x01, &qmi_technology);
	qmi_add_tlv(&msg, 0x14, strlen(apn), apn);
	if (*user) {
		qmi_add_tlv(&msg, 0x16, 0x01, &qmi_authentication);
			 ...
			 size_t count, loff_t *pos)
{
	...
	if (!strncmp(cmd, "verbose", 7)) {
		verbose = 1;
	} else if (!strncmp(cmd, "config:auth", 11) && strlen(cmd) == 13) {
		if (cmd[12] == '0')
			qmi_authentication = 0;
		if (cmd[12] == '1')
			qmi_authentication = 1;
		if (cmd[12] == '2')
			qmi_authentication = 2;
		if (cmd[12] == '3')
			qmi_authentication = 3;
	} else if (!strncmp(cmd, "config:tech", 11) && strlen(cmd) == 13) {
		if (cmd[12] == '1')
			qmi_technology = 1;
		if (cmd[12] == '2')
			qmi_technology = 2;
